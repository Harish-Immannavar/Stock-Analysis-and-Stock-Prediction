...
# 1. Get the absolute path of the 'Time series Forecasting' root folder
root_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# 2. Inject it into Python's search path if it isn't already there
if root_path not in sys.path:
    sys.path.insert(0, root_path)

# 3. Now try the absolute import
from pages.utils.plotly_figures import plotly_table , candlestick,MACD,RSI,close_chart,Moving_average
...
with col1:
    # 1. Properly initialize the DataFrame with a named column
    df = pd.DataFrame(
        data=[stock.info["marketCap"], stock.info["beta"], stock.info["trailingEps"], stock.info["trailingPE"]],
        index=['Market Cap', 'Beta', 'EPS', 'PE Ratio'],
        columns=['Value'] # Named column
    )
...
col1.metric("Daily Change",str(round(data['Close'].iloc[-1].squeeze(),2)),str(round(daily_change.squeeze(),2)))
